...
public class ImageListDiffer
{
    ...
    private readonly Func<IEnumerable<UiImage>>? _getImages;
    private List<ImageRenderState> _currentState = [];

    public ImageListDiffer(UiImageList imageList, Func<IEnumerable<UiImage>>? getImages = null)
    {
        _imageList = imageList;
        _getImages = getImages ?? (() => _imageList.Images);
    }
    ...
    public ListViewDiffs<UiImage> GetAndFlushDiffs()
    {
        lock (this)
        {
            ...
            lock (_imageList)
            {
                newState = _getImages().Select(x => x.GetImageRenderState()).ToList();
            }
