...
public abstract class DesktopForm : EtoFormBase
{
    ...
    private readonly DocumentTreeView _documentTreeView;
    ...
    private Guid? _currentFilterGroupId;
    private readonly ButtonMenuItem _splitItem;
    ...
    public DesktopForm(
        ...
        DocumentManager documentManager) : base(config)
    {
        ...
        _documentTreeView = new DocumentTreeView(documentManager, this, ImageList, _imageTransfer);
        ...
        _splitItem = new ButtonMenuItem { Text = "Split to New Document" };
        _splitItem.Click += SplitItem_Click;
        _contextMenu.Items.Add(_splitItem);
    ...
    {
        base.OnLoad(e);
        _imageListSyncer = new ImageListSyncer(ImageList, _listView.ApplyDiffs, SynchronizationContext.Current!, GetFilteredImages);
    ...
    protected virtual void CreateToolbarsAndMenus()
    {
        ...
        if (!hiddenButtons.HasFlag(ToolbarButtons.SavePdf))
            CreateToolbarButtonWithMenu(Commands.SavePdf, DesktopToolbarMenuType.SavePdf, new MenuProvider()
                ...
                .Append(Commands.SaveAllByGroup)
                .Separator()
    ...
    {
        var selectedImage = ImageList.Selection.FirstOrDefault();
        if (selectedImage != null)
        {
            var newGroup = _documentManager.SplitAtImage(selectedImage);
            if (newGroup != null)
            {
                _documentTreeView.SelectGroup(newGroup.Id);
            }
        }
    }

    private IEnumerable<UiImage> GetFilteredImages()
    {
        if (_currentFilterGroupId == null) return ImageList.Images;
        return ImageList.Images.Where(i => i.DocumentGroupId == _currentFilterGroupId.Value);
    }

    public void FilterToDocumentGroup(Guid? groupId)
    {
        if (_currentFilterGroupId == groupId) return;
        _currentFilterGroupId = groupId;
        _listView.SetItems(Array.Empty<UiImage>());
        _imageListSyncer?.Dispose();
        _imageListSyncer = new ImageListSyncer(ImageList, _listView.ApplyDiffs, SynchronizationContext.Current!, GetFilteredImages);
    }
}