...
public class EtoDialogHelper : DialogHelper
{
    ...
    public override bool PromptToSelectFolder(string? defaultPath, out string? savePath)
    {
        var sfd = new SelectFolderDialog();
        if (!string.IsNullOrEmpty(defaultPath))
        {
            var path = Path.IsPathRooted(defaultPath)
                ? Path.GetDirectoryName(NormalizePath(defaultPath))
                : defaultPath;
            if (path != null)
            {
                sfd.Directory = Path.GetFullPath(path);
            }
        }
        if (sfd.ShowDialog(null) == DialogResult.Ok)
        {
            savePath = sfd.Directory;
            return true;
        }
        savePath = null;
        return false;
