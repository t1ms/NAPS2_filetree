    ...
    private static string FindPath(string libraryName, string? baseFolder, string[] systemSearchPaths)
    {
        ...
        foreach (var actualBaseFolder in baseFolders)
        {
            foreach (var searchPath in systemSearchPaths)
            {
                ...
                searchedPaths.Add($"{path} (exists={File.Exists(path)})");
                if (File.Exists(path))
                {
                    return path;
                }
        ...
        System.Console.Error.WriteLine($"[NativeLibrary] Could not find '{libraryName}'. LibFolder='{AssemblyHelper.LibFolder}', EntryFolder='{AssemblyHelper.EntryFolder}'. Searched: {string.Join("; ", searchedPaths)}");
